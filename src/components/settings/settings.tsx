...
function Settings({
  ...
  // Get the current provider's models if available
  const providerModels = provider && provider !== "auto" && PROVIDERS[provider]?.models;
  
  // Set default model when provider changes
  useEffect(() => {
    if (provider !== "auto" && PROVIDERS[provider]?.models && onModelChange) {
      if (!model || !PROVIDERS[provider]?.models?.includes(model)) {
        onModelChange(PROVIDERS[provider].defaultModel);
      }
    }
  ...
  const handleProviderChange = (newProvider: string) => {
    ...
    // When switching to a provider with models, automatically select its default model
    if (newProvider !== "auto" && PROVIDERS[newProvider]?.models && onModelChange) {
      onModelChange(PROVIDERS[newProvider].defaultModel);
    }
  };
    ...
    <div className="">
      ...
      >
        ...
        <main className="px-4 pt-3 pb-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div
                ...
                onClick={() => {
                  handleProviderChange(provider === "auto" ? "fireworks-ai" : "auto");
                }}
              >
                <div
                  className={classNames(
          ...
          {/* Only show model selection if the provider has models */}
          {providerModels && onModelChange && (
            <label className="block">
              <p className="text-gray-800 text-sm font-medium mb-2 flex items-center justify-between">
                Model
              </p>
              <select 
                ...
                className="w-full rounded-md border border-gray-300 py-2 px-3 text-sm text-gray-700"
              >
                {providerModels.map((modelName: string) => (
                  <option key={modelName} value={modelName}>{modelName}</option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-2">
                Select which model to use for this provider
              </p>
            </label>
          )}
        </main>
